...
const showSuccessAlert = (message) => {
  ...
  const handleStarClick = async (starIndex) => {
    ...
    const  sum= newrate?.reduce((acc, curr) => parseInt(acc) + parseInt(curr), 0);
    ...
    const NupdatedCard=   await axios.put(`http://localhost:5000/api/games/${cardId}`, updatedCard);
    console.log(NupdatedCard.data)
